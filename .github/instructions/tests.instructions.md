...
- Test through the component/public surface; find elements with official test-utils
  ...
  is wired correctly; test-utils are the stable contract. EXCEPTION: when no test-utils finder exists
  for the element/state (e.g. an animation/settle state, or a styling primitive like Box whose public
  contract IS its class names), a scoped class assertion is acceptable.
