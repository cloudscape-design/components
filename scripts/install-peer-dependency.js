...
try {
  execCommand('npm run prepare --if-present');
} catch {
  console.log('No prepare script found or prepare failed, continuing...');
}
...
for (const moduleName of getModules(packageName)) {
  ...
  const targetNodeModules = path.join(modulePath, 'node_modules');
  console.log(`Copying dependencies for ${moduleName}...`);
  execCommand(`cp -R ${sourceNodeModules} ${targetNodeModules}`);
}
