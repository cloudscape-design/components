  ...
  describe('custom buttons', () => {
    ...
    test('passes expected arguments to custom row action', () => {
      ...
      expect(actionRenderer).toHaveBeenNthCalledWith(
        1,
        expect.objectContaining({ item: defaultProps.items![0], itemIndex: 0, ref: expect.any(Function) })
      );
      expect(actionRenderer).toHaveBeenNthCalledWith(
        2,
        expect.objectContaining({ item: defaultProps.items![1], itemIndex: 1, ref: expect.any(Function) })
      );
      expect(actionRenderer).toHaveBeenNthCalledWith(
        3,
        expect.objectContaining({ item: defaultProps.items![2], itemIndex: 2, ref: expect.any(Function) })
      );
    });
