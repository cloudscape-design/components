...
// No-op sink for the missing-css ops metric.
const metrics = {
  sendOpsMetricObject: (_metricName: string, _detail: Record<string, string | number | boolean>) => {},
};

function checkMissingStyles(ownerDocument: Document) {
  if (!GIT_SHA) {
    // GIT_SHA is empty (this build never stamps one), so the version-info CSS custom property
    // can never match. Skip the check rather than report a false-positive missing-styles error
    // on every page load.
    return;
  }
