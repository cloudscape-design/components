...
describe('position=viewport', () => {
  test('applies position:fixed', () => {
    const el = getDrawerElement(<Drawer position="viewport" />);
    expect(el).toHaveStyle({ position: 'fixed' });
  });

  test('applies position-viewport class', () => {
    const el = getDrawerElement(<Drawer position="viewport" placement="bottom" />);
    expect(el.className).toContain(drawerStyles['position-viewport']);
  });

  test('applies offset insets', () => {
    const el = getDrawerElement(
      <Drawer position="viewport" placement="start" offset={{ start: 8, top: 4, bottom: 12 }} />
    );
