...
test('renders token editor with a single property', () => {
  ...
  // Token inputs
  expect(wrapper.findPropertyField().findControl()!.find('button')!.getElement()).toHaveAccessibleName('Property Name');
...
test('removes first property', () => {
  ...
  wrapper.findTokenRemoveActions(1).openDropdown();
  wrapper.findTokenRemoveActions(1).findItems()[0].click();
  wrapper.findSubmitButton().click();
...
test('removes second property', () => {
  ...
  wrapper.findTokenRemoveActions(2).openDropdown();
  wrapper.findTokenRemoveActions(2).findItems()[0].click();
  wrapper.findSubmitButton().click();
...
test('removes first property from group', () => {
  ...
  wrapper.findTokenRemoveActions(1).openDropdown();
  wrapper.findTokenRemoveActions(1).findItems()[1].click();
  wrapper.findSubmitButton().click();
