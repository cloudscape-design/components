...
const formatState = (value: unknown, fallback = 'Invalid value') => {
  function formatOne(valueEntry: unknown) {
    return typeof valueEntry === 'string' ? states[valueEntry] || fallback : fallback;
  }
...
const filteringProperties: readonly FilteringProperty[] = [
  {
    ...
    propertyLabel: 'State',
    operators: [
      { operator: '=', format: formatState, tokenType: 'enum' },
      { operator: '!=', format: formatState, tokenType: 'enum' },
      { operator: ':', format: value => formatState(value, value) },
      { operator: '!:', format: value => formatState(value, value) },
    ],
...
const renderComponent = (props?: Partial<PropertyFilterProps>) => {
  ...
  const getTokensContent = () => wrapper.findTokens().map(w => w.getElement().textContent);
...
describe('sync token creation', () => {
  ...
  test('creates an enum token for state by submitting non-matched value with Enter', () => {
    ...
    expect(getTokensContent()).toEqual(['State = Invalid value']);
    expect(onChange).toHaveBeenCalledWith(
      expect.objectContaining({
