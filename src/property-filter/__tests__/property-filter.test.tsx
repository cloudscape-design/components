...
describe('property filter parts', () => {
  ...
  describe('dropdown states', () => {
    ...
    it('when free text filtering is not allowed and no property is matched, dropdown shows empty state and aria-expanded is true', () => {
      const { propertyFilterWrapper: wrapper } = renderComponent({
        disableFreeTextFiltering: true,
        filteringEmpty: 'No options',
      });
      wrapper.setInputValue('free-text');
      expect(wrapper.findNativeInput().getElement()).toHaveAttribute('aria-expanded', 'true');
      expect(wrapper.findDropdown().findOpenDropdown()!.getElement()).toHaveTextContent('No options');
    });
  });
