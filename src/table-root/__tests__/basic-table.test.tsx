...
describe('Table atomic parts', () => {
  ...
  describe('grid column layout', () => {
    ...
    test('the header row is aria-rowindex 1 only when ariaRowcount is provided (explicit row numbering)', () => {
      const withRowcount = renderTable({ grid: true, ariaRowcount: 500 });
      const withRowcountHeaderRow = withRowcount.wrapper.findTableHead()!.find('[role="row"]')!.getElement();
      expect(withRowcountHeaderRow.getAttribute('aria-rowindex')).toBe('1');

      // Without ariaRowcount the consumer isn't managing row numbering, so positions derive from the DOM and
      // no aria-rowindex is set.
