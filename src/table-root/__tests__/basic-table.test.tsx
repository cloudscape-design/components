...
describe('Table atomic parts', () => {
  ...
  describe('grid column layout', () => {
    ...
    test('the header row is aria-rowindex 1 only when the grid declares an aria-rowcount (virtualized)', () => {
      ...
      expect(virtualizedHeaderRow.getAttribute('aria-rowindex')).toBe('1');

      // In a non-virtualized grid the row positions derive from the DOM, so no aria-rowindex is set.
