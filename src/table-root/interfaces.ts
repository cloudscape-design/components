...
export interface TableRootProps extends BaseComponentProps {
  ...
  /**
   * The total number of rows on the table's `aria-rowcount`, counting the header row (so it is the
   * dataset row count plus 1, matching `TableRow.ariaRowindex`, which numbers data rows from 2).
   * Provide it only when you render a subset of rows, such as with virtualization, so assistive
   * technologies report the whole table. Omit it when you render every row and the count is derived
   * from the DOM.
   */
  ariaRowcount?: number;
}
