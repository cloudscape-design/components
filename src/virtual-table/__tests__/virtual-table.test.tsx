...
// Behavioural coverage for the compound VirtualTable core (impl-F1-A2-core), exercised through
// the public compound namespace + the generated wrapper. Axe / keyboard / SR coverage lives in
// impl-F1-A2-tests-a11y; this suite pins windowing, the windowed row-template invocation,
// header<->body reconciliation, the materialised disclosure column, controlled/uncontrolled
// expansion, sort activation, and the imperative ref.

// warnOnce is mocked so the reconciliation + fail-loud dev warnings can be asserted (repo
// dev-warnings convention). Built-lib component code resolves the same module specifier, so
// the mock intercepts the compiled Row reconciliation path too.
          ...
          return (
            <VirtualTable.Row item={item} api={api}>
              ...
              {/* ExpandedContent is declared unconditionally (not gated on api.isExpanded) per
                  the BodyProps contract; Root mounts the region only for expanded rows. */}
              {options.expandable && (
...
describe('VirtualTable (F1-A2 compound)', () => {
  ...
  test('invokes the Body row template only for windowed rows, not the whole dataset', () => {
    ...
    // this is how the compound API virtualizes without the consumer materializing every Row.
  ...
  test('dev-warns (fail-loud) when a row is expanded but the template rendered no ExpandedContent', () => {
    // expandable=false -> no ExpandedContent declared -> the probe finds no disclosure column,
    // but a controlled-expanded row still asks Root to open: fail loudly, do not silently
    // render nothing (B1 fail-loud guard).
    renderTable({ count: 5, expandable: false, controlledExpanded: ['row-0'] });
    expect(warnOnce).toHaveBeenCalledWith(
      'VirtualTable',
      expect.stringContaining('Declare ExpandedContent unconditionally')
    );
  });
