...
// The logs-specialized F2-A1 core drives the SAME windowing + opt-in measurement engine the
// F1 cells use: one owned inner scroll container, data rows fixed at the density estimate
// (no measurement cost) unless getRowHeight opts them into 'auto' (wrapping raw lines,
// CW-15), and only expanded regions / opted-in rows measured, with anchor-based offset
// correction so measured growth above the fold does not shift the viewport. jsdom has no
// layout, so the DOM-driven paths are exercised with a controllable ResizeObserver and a
// fake scroll container whose scrollTop/clientHeight/scrollHeight are stubbed (viewport
// falls back to 600px when clientHeight is 0). Because the engine windows over whatever
// array it is handed, a subset filter's filtered array makes aria-rowcount the filtered
// length automatically (verified at the component level).
...
describe('VirtualTable (F2-A1) useVirtualModel', () => {
  ...
  test('does not observe fixed-height rows but does observe auto rows, and applies the measured height', () => {
    ...
    // An auto data row is observed (the F2-A1 getRowHeight='auto' path that lets the raw view
