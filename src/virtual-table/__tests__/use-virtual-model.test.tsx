...
// jsdom has no layout engine, so the engine's DOM-driven paths (viewport height,
// element measurement) are exercised with a controllable ResizeObserver and a fake
// scroll container whose scrollTop/clientHeight/scrollHeight are stubbed. The default
// viewport falls back to 600px when clientHeight is 0 (see use-virtual-model).
...
describe('VirtualTable useVirtualModel', () => {
  ...
  test('recomputes the visible range when the container scrolls', () => {
    ...
    // Scroll to offset 4000 (row 200). The scroll listener re-reads scrollTop and the
    // window recomputes: firstVisible 200 - 5 overscan = 195; lastVisible 230 + 5 = 235.
    act(() => {
  ...
  test('inserts an expanded slot immediately after its data row', () => {
    ...
    // Runway grows by exactly the expanded region height.
  ...
  test('does not observe fixed-height rows but does observe auto rows, and applies the measured height', () => {
    ...
    // observers[0] is the engine's own viewport ResizeObserver (attached on mount);
    // `before` is captured after mount so any new observer here is the measurement one.
    // A fixed row (auto=false) is never observed — it never pays measurement cost.
    act(() => result.current.measureRef('d:0', false)(stubHeight(55)));
    expect(observers.length).toBe(before);

    // An auto row is observed; firing the observer applies the real height.
