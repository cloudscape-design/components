...
describe('VirtualTable useExpansion', () => {
  ...
  test('controlled mode is driven by expandedItems, not internal state', () => {
    ...
    // Toggling in controlled mode emits collapse intent but must not mutate local state:
    // the controlled prop is unchanged, so the row stays expanded until the consumer updates it.
    act(() => result.current.toggle(a));
    expect(onExpandChange).toHaveBeenLastCalledWith(
      expect.objectContaining({ detail: expect.objectContaining({ expanded: false, expandedItems: [] }) })
    );
