...
// A11y tests for the COMPOUND VirtualTable (impl-F1-A2-tests-a11y). Placement mirrors the
// F1-A1 tests-a11y decision: the `__a11y__` directory is the node/puppeteer integ runner
// (`.ts`-only, tsconfig.integ), so axe + RTL keyboard/SR `.tsx` tests belong under
...
//
// The central a11y claim for the compound API is that the Root/Header/HeaderCell/Body/Row/
// Cell/ExpandedContent composition produces the SAME accessibility tree as a single monolithic
// grid: one role="grid" that owns only rowgroups -> rows -> columnheader/gridcell, with the
// materialised disclosure column, full-dataset aria-rowcount/rowindex + aria-colcount/colindex
// coherence, a labeled expanded region, and non-row chrome (loading/empty/live-region) OUTSIDE
// the grid. Coverage: axe/HTML validity, the compound-produces-one-grid contract, row-granular
// keyboard nav via aria-activedescendant (incl. ArrowLeft/Right inert), sort-trigger a11y
// (keyboard-operable + accessible name + aria-sort), disclosure activation, Tab-in / Escape-out
// of the expanded region, live-append debounce, and focus-under-recycling.
...
// Stable identity so useLiveAnnouncement's effect does not re-run (and reset coalescing) on
// unrelated rerenders — the compound Root threads this straight into the live-region hook.
...
// live-region hook identity) stays stable across renders.
        ...
        {(item: Item, api) => (
          <VirtualTable.Row item={item} api={api}>
            ...
            {/* ExpandedContent is declared UNCONDITIONALLY (not gated on api.isExpanded) per the
                BodyProps contract; Root mounts the region only for expanded rows. */}
            {options.expandable && (
...
describe('VirtualTable F1-A2 a11y', () => {
  describe('axe / HTML validity', () => {
    test('validates a plain compound grid without expansion', async () => {
  ...
  describe('compound structure produces a single grid accessibility tree', () => {
    test('the seven compound sub-components collapse to one grid -> rowgroup -> row tree', () => {
      ...
      // Exactly one grid, regardless of how many compound elements were authored.
  ...
  describe('keyboard navigation (aria-activedescendant grid model)', () => {
    test('the grid is a single always-present tab stop and seeds an active descendant', () => {
      ...
      // The active descendant is a keyboard-navigation concept, seeded only once the grid
      // actually holds focus; it then defaults to the first (rendered) row.
    ...
    test('focusing a descendant control does not advertise an active descendant on the grid', () => {
      const { container, wrapper } = renderTable(makeItems(20), { expandable: true });
      const grid = getGrid(container);
      // focusin bubbles from the disclosure button to the container; the guard must ignore
      // descendant focus so items[0] is not falsely activated (WCAG).
    ...
    test('ArrowLeft / ArrowRight are inert (deliberate row-granular grid, not 2D cell nav)', () => {
      // Like F1-A1, the compound grid is deliberately row-granular: aria-activedescendant
      // references a role="row" (never a gridcell) and onGridKeyDown handles only
      // Up/Down/Home/End. A legitimate APG row-as-widget variant of role="grid"; the horizontal
      // cell-navigation keys are intentionally inert. Carried to impl-F1-A2-docs as an explicit
      // a11y contract note.
      const { container, wrapper } = renderTable(makeItems(20));
      const grid = getGrid(container);
      fireEvent.focus(grid);
  ...
  describe('sort trigger accessibility', () => {
    test('sortable headers expose a keyboard-operable trigger with an accessible name; non-sortable do not', () => {
      ...
      // activateSortLabel (not the raw columnId announced bare).
