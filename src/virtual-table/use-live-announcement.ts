...
// Coalesces streaming appends into a single polite announcement per burst. The consumer
// decides WHAT to announce (via ariaLabels.appendAnnouncement); the component owns the
// debounce and the aria-live region so a high-rate live tail announces "N new log events"
// once per ~500ms batch, not once per row.
