...
// Windowing + opt-in measurement engine for the logs-specialized VirtualTable
// (impl-F2-A1-core). This is the SAME behaviour/a11y engine the F1 cells use — a single
// owned inner scroll container (design B4), the body windows within it, data rows are
// fixed at `estimatedRowHeight` (23 at CloudWatch grid density, 20 for raw) so a 100k
// fixed-row stream never pays ResizeObserver cost, and only 'auto' data rows (wrapping
// raw lines, CW-15) and expanded regions are measured (design B5). Measured growth of a
// slot above the fold is corrected by re-anchoring on the first visible data row's
// identity, so scroll position stays put as variable EXPANDED heights are discovered
// (no CW-3 drift). The component windows over whatever item array it is handed — in a
// `subset` filter that array is the filtered set, so aria-rowcount reflects the filtered
...
interface UseVirtualModelParams<T> {
  ...
  /** Pre-measurement runway estimate for an 'auto' expanded region (preset A ~300 / B ~150). */
...
export function useVirtualModel<T>({
  ...
  const layout = useMemo(() => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      // getRowHeight opts a row into 'auto', it is measured so wrapping rows (the raw view,
      // CW-15) window at their real height; a numeric return pins it.
