...
export function TabHeaderBar({
  ...
  return (
    <div className={classes} style={getTabHeaderStyles(style) as React.CSSProperties}>
    ...
    return (
      ...
      >
        <div
          ...
          style={getTabContainerStyles(style) as React.CSSProperties}
        >
          <TabTrigger
            ref={setElement}
            tab={tab}
...
const TabTrigger = forwardRef(
  ({ tab, elementProps, activeTabId, index, style }: TabTriggerProps, ref: React.Ref<HTMLElement>) => {
    ...
    const commonProps = {
      ...
      ref: mergedRef,
      tabIndex: tabIndex,
      style: { ...elementProps.style, ...getTabStyles(style) } as React.CSSProperties,
