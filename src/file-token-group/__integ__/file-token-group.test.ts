...
const fileTokenGroupWrapper = createWrapper().findFileTokenGroup('#test');

class FileTokenGroupPage extends BasePageObject {
  ...
  clickDismissButton(itemIndex: number) {
    return this.click(fileTokenGroupWrapper.findFileToken(itemIndex).findRemoveButton().getElement());
  }

  countItems() {
    return this.getElementsCount(fileTokenGroupWrapper.findFileTokens().toSelector());
  }
...
const setupTest = (testFn: (page: FileTokenGroupPage) => Promise<void>) => {
  return useBrowser(async browser => {
    const page = new FileTokenGroupPage(browser);
    await browser.url('#/light/file-token-group/index');
    await page.waitForVisible(fileTokenGroupWrapper.findFileToken(1).toSelector());
    await testFn(page);
  });
};
