...
function renderWithProvider(componentIcons: IconProviderProps.ComponentIcons, children: React.ReactNode) {
  return render(<IconProvider componentIcons={componentIcons}>{children}</IconProvider>);
}
...
describe('inheritance', () => {
  it('inherits parent icons when the icons property is omitted', () => {
    const CUSTOM_SVG = (
      <svg focusable={false} data-testid="custom-svg">
        <circle cx="8" cy="8" r="7" />
      </svg>
    );
    const { container } = render(
      <IconProvider icons={{ 'caret-down-filled': CUSTOM_SVG }}>
        <IconProvider componentIcons={{ table: {} }}>
          <ExpandableSection headerText="Section" />
        </IconProvider>
      </IconProvider>
    );
    // The nested provider omits `icons`, so the parent's toggle icon replacement still applies.
    expect(container.querySelector('[data-testid=custom-svg]')).toBeTruthy();
  });

  it('merges component icons across nested providers, closest provider wins', () => {
    const { queryByTestId } = render(
      <IconProvider componentIcons={{ 'expandable-section': { expandToggle } }}>
        <IconProvider componentIcons={{ 'expandable-section': { expandToggle: () => <span data-testid="inner" /> } }}>
          <ExpandableSection headerText="Section" />
        </IconProvider>
      </IconProvider>
    );
  ...
  it('resets a specific component when set to null', () => {
    const { queryByTestId } = render(
      <IconProvider componentIcons={{ 'expandable-section': { expandToggle }, 'tree-view': { expandToggle } }}>
        <IconProvider componentIcons={{ 'expandable-section': null }}>
          <ExpandableSection headerText="Section" />
        </IconProvider>
      </IconProvider>
    );
  ...
  it('resets all component icons when set to null', () => {
    const { queryByTestId } = render(
      <IconProvider componentIcons={{ 'expandable-section': { expandToggle } }}>
        <IconProvider componentIcons={null}>
          <ExpandableSection headerText="Section" />
        </IconProvider>
      </IconProvider>
    );
