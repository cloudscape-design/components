  ...
  describe.each(['iso', 'slashed', 'long-localized'] as DateRangePickerProps.AbsoluteFormat[])(
    ...
    absoluteFormat => {
      ...
      describe.each([...inputFormatsToTest] as DateRangePickerProps.DateInputFormat[])(
        ...
        dateInputFormat => {
          const isIso = absoluteFormat === 'iso' || (absoluteFormat === 'long-localized' && dateInputFormat === 'iso');
          ...
          describe('disabled date', () => {
            ...
            test('should be able to select enabled date', () => {
              ...
              expect(wrapper.findDropdown()!.findStartDateInput()!.findNativeInput().getElement().value).toBe(
                `2018${separator}03${separator}04`
              );
            });
            ...
            test('should not select disabled date', () => {
              ...
              expect(wrapper.findDropdown()!.findStartDateInput()!.findNativeInput().getElement().value).toBe(
                `2018${separator}03${separator}01`
              );
            });
            ...
            describe('disabled with reason', () => {
              ...
              test('open tooltip on focus', () => {
                ...
                expect(
                  wrapper.findDropdown()!.findDateAt('left', 4, 1).findDisabledReason()!.getElement()
              ...
              test('has no aria-describedby by default', () => {
                ...
                expect(wrapper.findDropdown()!.findDateAt('left', 4, 1).getElement()).not.toHaveAttribute(
                  'aria-describedby'
                );
              });
              ...
              test('has aria-describedby with disabledReason', () => {
                ...
                expect(wrapper.findDropdown()!.findDateAt('left', 4, 1).getElement()).toHaveAttribute(
                  'aria-describedby'
                );
              });
              ...
              test('has hidden element (linked to aria-describedby) with disabledReason', () => {
                ...
                expect(
                  wrapper.findDropdown()!.findDateAt('left', 4, 1).find('span[hidden]')!.getElement()
                ).toHaveTextContent('Disabled with a reason');
              });
              ...
              test('disabled with reason start date can not be selected on space keydown', () => {
                ...
                changeMode(wrapper, 'absolute');
                const startDateInputValue = `2018${separator}03${separator}01`;
          ...
          describe('aria labels', () => {
            test('should add `todayAriaLabel` to today in the calendar', () => {
              ...
              changeMode(wrapper, 'absolute');
              expect(
                wrapper.findDropdown()?.findCurrentDay().find(`.${screenreaderOnlyStyles.root}`)?.getElement()
                  .textContent
              ).toMatch('TEST TODAY');
            });
            ...
            test('should add aria-selected="true" to selected range in the calendar', () => {
              ...
              changeMode(wrapper, 'absolute');
              expect(wrapper.findDropdown()!.findSelectedStartDate()!.getElement().getAttribute('aria-selected')).toBe(
                'true'
              );
              expect(wrapper.findDropdown()!.findSelectedEndDate()!.getElement().getAttribute('aria-selected')).toBe(
                'true'
              );
              expect(wrapper.findDropdown()!.findDateAt('left', 2, 1).getElement().getAttribute('aria-selected')).toBe(
                'true'
              );
            });
            ...
            test('date/time inputs should have ariaDescribedby attribute when constriantText is available', () => {
              ...
              expect(wrapper.find(`#${constraindId}`)!.getElement()).toContainHTML(
                defaultProps.i18nStrings[isIso ? 'isoDateTimeConstraintText' : 'slashedDateTimeConstraintText'] || ''
              );
              expect(
                wrapper
                  .findDropdown()!
            ...
            test('renders default range announcement when i18n string is not provided', () => {
              ...
              expect(findLiveAnnouncement(wrapper)).toHaveTextContent(new RegExp(i18nStrings.endDateLabel!));
              expect(findLiveAnnouncement(wrapper)).toHaveTextContent(
                'Sunday, September 6, 2020 – Monday, September 7, 2020'
              );
            });
            ...
            test('should set aria-disabled="true" and unset aria-selected to disabled date', () => {
              ...
              changeMode(wrapper, 'absolute');
              expect(wrapper.findDropdown()!.findDateAt('left', 3, 3).getElement().getAttribute('aria-disabled')).toBe(
                'true'
              );
              expect(wrapper.findDropdown()!.findDateAt('left', 3, 3).getElement().getAttribute('aria-selected')).toBe(
                null
              );
            });
          });
