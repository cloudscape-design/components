...
describe('Date range picker', () => {
  describe.each(['day', 'month'] as const)('With granularity of %s', granularity => {
    ...
    //todo  fix
    testIf(false)('opens relative range mode by default', () => {
    ...
    describe('validation', () => {
      ...
      test('produces the value even if validation does not catch an incomplete range', () => {
        ...
        //todo  look into false passing both granularities
      ...
      //todo  fix test for month
      testIf(granularity === 'day')('should not fire onChange if the date is invalid', () => {
        wrapper.openDropdown();
    ...
    describe('range selector mode', () => {
      ...
      test('shows warning when range selector mode and value type are not compatible', () => {
        ...
        for (const [rangeSelectorMode, value] of compatible) {
          renderDateRangePicker({ ...updatedProps, rangeSelectorMode, value });
        }
        expect(warnOnce).not.toHaveBeenCalled();
    ...
    describe('isValidRange', () => {
      ...
      test('calls isValidRange without time part when dateOnly is enabled', () => {
        const expectedStartDate = granularity === 'month' ? `2019-09-01` : `2020-09-15`;
        // const expectedEndDate = granularity === 'month' ? '2020-09-01' : '2020-10-01';
