...
export default function InternalPagination({
  ...
  const prevLoadingRef = React.useRef(jumpToPage?.isLoading);
  ...
  React.useEffect(() => {
    if (prevLoadingRef.current && !jumpToPage?.isLoading) {
      setJumpToPageValue(String(currentPageIndex));
    }
    prevLoadingRef.current = jumpToPage?.isLoading;
  }, [jumpToPage?.isLoading, currentPageIndex]);
  ...
  const jumpToPageLabel =
    i18n('i8nStrings.jumpToPageLabel', i18nStrings?.jumpToPageLabel) ?? defaultI18nStrings.jumpToPageLabel;
  const jumpToPageButtonLabel =
    i18n('ariaLabels.jumpToPageButton', ariaLabels?.jumpToPageButton) ?? defaultAriaLabels.jumpToPageButton;
  ...
  function handleJumpToPageClick(requestedPageIndex: number) {
    if (openEnd) {
      ...
      if (requestedPageIndex >= 1 && requestedPageIndex <= pagesCount) {
        ...
        fireNonCancelableEvent(onChange, { currentPageIndex: pagesCount });
      }
    }
  }
  ...
  React.useEffect(() => {
    const hasError = internalHasError || jumpToPage?.hasError;
    if (hasError) {
      // For open-end, wait until loading completes
      if (openEnd && jumpToPage?.isLoading) {
        return;
      }
  ...
  }, [internalHasError, jumpToPage?.hasError, jumpToPage?.isLoading, openEnd]);
      ...
      {jumpToPage && (
        <div className={styles['jump-to-page']}>
          <InternalSpaceBetween size="xxs" direction="horizontal" alignItems="end">
            ...
            {internalHasError || jumpToPage?.hasError ? (
              <InternalPopover
                size="medium"
                dismissButton={false}
