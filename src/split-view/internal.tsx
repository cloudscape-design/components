  ...
  (
    ...
    return (
      <div
        {...baseProps}
        ref={mergedRef}
        className={clsx(baseProps.className, styles.root, testStyles.root, styles[`position-${panelPosition}`])}
      >
        <div className={clsx(styles.content, display === 'panel-only' ? styles['content-hidden'] : testStyles.content)}>
          {mainContent}
        </div>
        <div
          className={clsx(
            ...
            display === 'main-only' ? styles['panel-hidden'] : testStyles.panel
          )}
          ref={panelRef}
          style={display === 'all' ? { inlineSize: `${panelSize}px` } : undefined}
        >
          <div className={styles['panel-content']}>{panelContent}</div>
