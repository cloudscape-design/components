...
describe('SplitView Component', () => {
  ...
  describe('Accessibility', () => {
    test('resize handle has proper aria attributes', () => {
      ...
      expect(handle!.getElement()).toHaveAttribute('aria-valuenow');
    });
  });
  ...
  describe('Display property', () => {
    test('renders both content and panel when display is "all" (default)', () => {
      ...
      expect(content!.getElement()).not.toHaveClass(styles['content-hidden']);
      expect(panel!.getElement()).not.toHaveClass(styles['panel-hidden']);
    });
