...
export default class VirtualTableWrapper extends ComponentWrapper {
   ...
   * `aria-rowindex`, which is the full-dataset index offset by the header row: the header
   * is `aria-rowindex=1`, so data row `dataIndex` is `aria-rowindex=dataIndex+2`. Indexing
   * this way (not by position in the windowed array) is correct under virtualization.
   */
