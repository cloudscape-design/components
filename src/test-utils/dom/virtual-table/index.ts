...
export default class VirtualTableWrapper extends ComponentWrapper {
   ...
   * `aria-rowindex`, which is the full-dataset index offset by the header row: header
   * is `aria-rowindex=1`, so data row `dataIndex` is `aria-rowindex=dataIndex+2`.
   */
  findRowByIndex(dataIndex: number): ElementWrapper | null {
    return this.find(`.${styles.row}[aria-rowindex="${dataIndex + 2}"]`);
  }

  /** The disclosure toggle button for a data row, or null if outside the window. */
  ...
  /** The header row element (returned regardless of the stickyHeader prop). */
  findHeaderRow(): ElementWrapper | null {
    return this.findByClassName(styles['header-row']);
  }

  /** The live-tail follow toggle, or null when the live-tail seam is not wired. */
  findFollowToggle(): ElementWrapper | null {
    return this.findByClassName(styles['follow-toggle']);
  }

  /**
   * The total-vs-filtered match-count region, or null when no filter is set. Rendered
   * for both filter modes (subset shows the filtered length, mark-in-place the match
   * count) under the `match-count` class.
   */
  findMatchCount(): ElementWrapper | null {
    return this.findByClassName(styles['match-count']);
  }
