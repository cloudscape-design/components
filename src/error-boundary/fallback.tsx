...
function DefaultDescriptionContent({
  ...
  if (Feedback) {
    ...
    const providedDescription: undefined | React.ReactNode = descriptionText
      ? new IntlMessageFormat(descriptionText).format(components)
      : undefined;
    ...
    const message = i18n('i18nStrings.descriptionWithFeedbackText', providedDescription, format => format(components));
    return (
      <>
  ...
  return <>{i18n('i18nStrings.descriptionText', descriptionText)}</>;
}
