...
export const InternalI18nContext = React.createContext<InternalI18nContextProps>({
  locale: null,
  format<T>(_namespace: string, _component: string, _key: string, provided: T) {
    return provided;
  },
});
...
export function getDocumentLanguage(): string | null {
  return typeof document !== 'undefined' ? document.documentElement.lang || null : null;
}
...
export function useLocale(): string {
  return useContext(InternalI18nContext).locale ?? getDocumentLanguage() ?? getBrowserLocale();
}
