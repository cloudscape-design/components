...
export interface TableRowProps extends BaseComponentProps {
  /**
   * The row's visual state. This is visual only — convey selection to assistive technologies with the
   * selection control (a checkbox or radio) in a leading cell.
   * * `default` - A standard row.
   * * `selected` - Applies the selected-row styling. Pair it with a selection control, such as a
   *   checkbox, in a leading cell.
