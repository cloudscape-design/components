...
export default function InternalHeader({
  ...
  const variantOverride = variant === 'awsui-h1-sticky' ? dynamicVariant : variant;
  ...
  return (
    ...
    >
      ...
      >
        <div className={clsx(styles.title, styles[`title-variant-${variantOverride}`], styles.refresh)}>
          <HeadingTag
...
export function Description({ children, variantOverride }: { children: React.ReactNode; variantOverride: string }) {
  return (
    (children && (
      <p className={clsx(styles.description, styles[`description-variant-${variantOverride}`], styles.refresh)}>
        {children}
      </p>
    )) ||
    null
