    ...
    <table role="grid" ref={tableRef}>
      ...
      <tbody>
        <tr aria-rowindex={2}>
          ...
          <td aria-colindex={2}>cell-1-2</td>
          <td aria-colindex={3} data-dialog-cell={actionsDialog}>
...
test('supports dialog cell navigation', () => {
...
test('when focus is inside dialog cell the cell and the cell next to it are focusable', () => {
  const { container } = render(<InteractiveTable actionsDialog={true} />);
  const table = container.querySelector('table')!;
  const dialogCell = container.querySelector('[aria-rowindex="2"]')!.querySelector('[aria-colindex="3"]')!;
  const nextCell = container.querySelector('[aria-rowindex="3"]')!.querySelector('[aria-colindex="1"]')!;

  dialogCell.querySelector('button')?.focus();

  expect(dialogCell).toHaveAttribute('tabIndex', '0');
