...
export class ItemSelectionTree<T> {
  ...
  constructor(selectionInverted: boolean, selectedItems: readonly T[], treeProps: TreeProps<T>) {
    ...
    const traverse = (item: T) => {
      ...
      // console.log((item as any).key, (item as any).children.length, treeProps.getChildren(item).length);
    };
  ...
  invertAll = (): ItemSelectionTree<T> => {
    const clone = this.clone();
    clone.toggleKey(rootItemKey);
    clone.treeProps.rootItems.forEach(item => clone.toggleKey(clone.getKey(item)));
    clone.computeState();
    return clone;
  };
  ...
  invertOne = (item: T): ItemSelectionTree<T> => {
    const clone = this.clone();
