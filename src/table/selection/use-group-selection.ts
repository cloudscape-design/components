...
export function useGroupSelection<T>({
  ...
  getLoaderSelectionProps?: (item: null | T) => SelectionProps;
} {
  ...
  const createLoaderToggleHandle = (item: null | T) => () => {
    if (!item) {
      fireNonCancelableEvent(onSelectionChange, selectionTree.invertAll().getState());
    } else {
      fireNonCancelableEvent(onSelectionChange, selectionTree.invertOne(item).getState());
    }
    setLastUserAction?.('selection');
  };
  ...
  return {
    ...
    getLoaderSelectionProps: (item: null | T): SelectionProps => ({
      ...
      selectionType: 'multi',
      disabled: false,
      checked: item ? selectionTree.isItemSelected(item) : selectionInverted,
      indeterminate: item ? selectionTree.isItemIndeterminate(item) : selectionTree.isSomeItemsSelected(),
      ...
      ariaLabel: item
        ...
        : joinStrings(
            ariaLabels?.selectionGroupLabel,
            ariaLabels?.allItemsLoaderSelectionLabel?.({ selectedItems, selectionInverted }) ??
              ariaLabels?.allItemsSelectionLabel?.({ selectedItems, selectionInverted })
          ),
    }),
  };
