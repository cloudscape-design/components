...
test(
  ...
  useBrowser(async browser => {
    ...
    const page = new BasePageObject(browser);
    await expect(browser.execute(extractHeight, header)).resolves.toEqual({ height: '103px', marginBottom: '0px' });
    await page.windowScrollTo({ top: 100 });
    await page.waitForJsTimers();
    await expect(browser.execute(extractHeight, header)).resolves.toEqual({ height: '93px', marginBottom: '10px' });
  })
);
