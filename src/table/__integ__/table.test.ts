...
test(
  ...
  useBrowser(async browser => {
    ...
    const page = new BasePageObject(browser);
    await expect(browser.execute(extractHeight, header)).resolves.toEqual({ height: '82px', marginBottom: '0px' });
    await page.windowScrollTo({ top: 100 });
    await page.waitForJsTimers();
    await expect(browser.execute(extractHeight, header)).resolves.toEqual({ height: '79px', marginBottom: '6px' });
  })
);
