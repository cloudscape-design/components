...
describe.each([false, true])('enableKeyboardNavigation=%s', enableKeyboardNavigation => {
  ...
  test.each(['Enter', 'Space'])('can start editing with %s key', async key => {
    await setupTest({ enableKeyboardNavigation }, async page => {
      ...
      await expect(page.isFocused(cellEditButton$(...domainNameRow1))).resolves.toBe(true);
