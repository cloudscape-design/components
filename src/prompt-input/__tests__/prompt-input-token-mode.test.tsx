...
describe('switching between textarea mode and token mode after mount', () => {
  // Regression for V2256683277: when PromptInput mounts in value-controlled
  // (textarea) mode and later switches to token mode (e.g. an async feature flag
  // resolves), the contentEditable element only mounts on the switch. Effects that
  // wire up the CaretController and editing listeners must re-run when that element
  // mounts — otherwise the caret controller is never built and typing misbehaves
  // (characters inserted at offset 0 / reversed input).
