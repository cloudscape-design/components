...
export function AppLayoutDrawerImplementation({ appLayoutInternals }: AppLayoutDrawerImplementationProps) {
  ...
  useEffect(() => {
    const { resizable } = activeDrawer || {};
    const { slider, close } = drawersFocusControl?.refs || {};

    if (activeDrawer) {
      const focusElement = resizable ? slider?.current : close?.current;
      focusElement?.focus();
    }
  }, [activeDrawer, drawersFocusControl?.refs]);
