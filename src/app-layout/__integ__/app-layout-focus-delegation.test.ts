...
class FocusDelegationPage extends BasePageObject {
  getActiveElement() {
...
function setupTest(
  testFn: (page: FocusDelegationPage) => Promise<void>,
  {
    pageName = 'with-split-panel',
    visualRefresh = false,
    splitPanelPosition = '',
    mobile = false,
  ...
  return useBrowser(async browser => {
    ...
    }${appLayoutWidget ? `&appLayoutWidget=${appLayoutWidget}` : ''}`;
    const page = new FocusDelegationPage(browser);
  ...
  describe(`visualRefresh=${visualRefresh}`, () => {
    [true, false].forEach(mobile =>
      describe(`mobile=${mobile}`, () => {
        ...
        [true, false].forEach(appLayoutWidget => {
          describe(`appLayoutWidget=${appLayoutWidget}`, () => {
            //     test(
            ...
            //           await expect(page.isFocused(triggerSelector)).resolves.toBe(true);
            //         },
            //         { pageName: 'with-drawers', visualRefresh, mobile, appLayoutWidget }
            //       )
            //     );

            //     describe('drawer focus interaction with buttons', () => {
            //       testIf(!appLayoutWidget)(
            ...
            testIf(appLayoutWidget)(
              ...
              setupTest(
                async page => {
                  const triggerSelector = wrapper.findDrawerTriggerById('pro-help').toSelector();
                  await page.click(triggerSelector);
                  //this will only apply to drawers that are not resizable
                  await expect(page.isFocused(wrapper.findActiveDrawerCloseButton().toSelector())).resolves.toBe(true);
                  const k = await page.getActiveElement();
                  console.log(k);
                  await page.keys('Enter');
                  await expect(page.isDisplayed(wrapper.findActiveDrawer().toSelector())).resolves.toBeFalsy();
                  const t = await page.getActiveElement();
                  console.log(t);
