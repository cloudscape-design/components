...
Conventions for writing and reviewing code in this repo live in
[.github/copilot-instructions.md](.github/copilot-instructions.md) (repo-wide) and
[.github/instructions/](.github/instructions/) (file-scoped: component structure & public API, tests,
SCSS, dev pages, test-utils). Tools that support these files apply them automatically; they also serve as
the review bar for hand-written code. Follow them when writing or changing code here.

## Conventions to watch

- **PR titles: `type: subject`, no scope.** The PR-title lint (`cloudscape-design/actions/.github/workflows/lint-pr.yml`) allows exactly these types: `chore`, `feat`, `fix`, `refactor`, `test`, `revert`. The title must start with `type:` followed by the subject (for example `feat: Add multi-column sort`). Scope parentheses are not supported (`feat(table): …` fails the check), and other Conventional Commits types such as `docs` or `style` are not allowed — use `chore:` for documentation and tooling changes. Only the PR title is checked; individual commit messages are not.
