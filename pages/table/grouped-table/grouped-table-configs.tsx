  ...
  return [
    {
      ...
      cell: item => {
        const counterText = getCounterText(item);
        return (
          <SpaceBetween direction="horizontal" size="xs" alignItems="center">
            ...
            {item.children.length > 0 && counterText && (
              <Box key="counter" color="text-body-secondary" fontSize="body-s">
                {counterText}
              </Box>
            )}
          </SpaceBetween>
    ...
    },
    {
      ...
      cell: item =>
        typeof item.amountEur === 'number'
          ? eurFormatter(item.amountEur)
          : `${eurFormatter(item.amountEur.totalAmount)} / ${eurFormatter(item.amountEur.averageAmount)}`,
    ...
    },
    {
      ...
      cell: item =>
        typeof item.amountUsd === 'number'
          ? usdFormatter(item.amountUsd)
          : `${usdFormatter(item.amountUsd.totalAmount)} / ${usdFormatter(item.amountUsd.averageAmount)}`,
