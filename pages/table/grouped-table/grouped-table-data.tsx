...
repeat(
  transfer('Lovers International', 'Jane Doe', 'USD', () => 4500),
  monthly
);
...
export function getGroupedTransactions(
  ...
  collectionProps: UseCollectionResult<Transaction>['collectionProps']
): TransactionRow[] {
  function makeGroups(
    ...
    const { sortingColumn, sortingDescending } = collectionProps;
    const tableSortingDirection = ['amountEur', 'amountUsd'].includes(sortingColumn?.sortingField ?? '')
      ? sortingDescending
        ? 'desc'
        : 'asc'
      : undefined;
    ...
    const rows = orderBy(
      Object.entries(byProperty).map(([groupKey, groupTransactions]) => {
        ...
        return {
          ...
          __sortingProperty: (() => {
            const tableSortingField = collectionProps.sortingColumn?.sortingField;
            if (tableSortingField === 'amountEur') {
              return sumBy(groupTransactions, 'amountEur');
            }
            if (tableSortingField === 'amountUsd') {
              return sumBy(groupTransactions, 'amountUsd');
            }
            return (groupTransactions[0] as any)[group.property];
          })(),
        } as TransactionRow & { __sortingProperty: any };
      }),
      '__sortingProperty',
      tableSortingDirection ?? group.sorting
    );
    return rows;
  }
