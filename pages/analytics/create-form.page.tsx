...
function Content() {
  ...
  const [stepErrorAlertVisible, setStepErrorAlertVisible] = useState(false);
  const [subStepErrorAlertVisible, setSubStepErrorAlertVisible] = useState(false);
  const [funnelErrorAlertVisible, setFunnelErrorAlertVisible] = useState(false);
  ...
  return (
    <>
      {funnelErrorAlertVisible && (
        <Alert statusIconAriaLabel="Error" type="error" header="Funnel error">
          This is a funnel error
        </Alert>
      )}
      <Form
        analyticsMetadata={{
        ...
        actions={
          <SpaceBetween direction="horizontal" size="xs">
            <Button
              data-testid="toggle-funnel-error-button"
              onClick={() => setFunnelErrorAlertVisible(prevValue => !prevValue)}
            >
              Toggle funnel error alert
            </Button>
            <Button
              data-testid="toggle-step-error-button"
              onClick={() => setStepErrorAlertVisible(prevValue => !prevValue)}
            >
              Toggle step error alert
            </Button>
      ...
      >
        ...
        <SpaceBetween size="s">
          {containersVisible[0] && (
            ...
            >
              <SpaceBetween size="s">
                ...
                <Button
                  data-testid="toggle-substep-error-button"
                  onClick={() => setSubStepErrorAlertVisible(prevValue => !prevValue)}
                >
                  Toggle substep error alert
                </Button>
                ...
                {subStepErrorAlertVisible && (
                  <Alert statusIconAriaLabel="Error" type="error" header="Substep error">
                    This is a substep error
                  </Alert>
                )}
              </SpaceBetween>
            </Container>
          ...
          {stepErrorAlertVisible && (
            <Alert statusIconAriaLabel="Error" type="error" header="Step error">
              This is a step error
            </Alert>
          )}
