...
export function DateRangePickerCustom() {
  ...
  const [pendingTimezone, setPendingTimezone] = useState<SelectProps.Option>(timezoneOptions[0]);
  ...
  const renderRelativeRangeContent = (
    ...
    const selectedKey = getSelectedKey(selectedRange);

    return (
      <SpaceBetween size="l">
        <FormField label="Relative range">
          <ul className={styles['option-list']} role="listbox" aria-label="Relative range">
            {relativeOptions.map(o => {
              const isSelected = !isCustomSelected && o.value.key === selectedKey;
              return (
                <li
                  key={o.value.key}
                  role="option"
                  aria-selected={isSelected}
                  className={`${styles['option-item']} ${isSelected ? styles['option-item-selected'] : ''}`}
                  onClick={() => {
                    setIsCustomSelected(false);
                    setSelectedRange(o.value);
                  }}
                >
                  {o.label}
                </li>
            ...
            <li
              ...
              className={`${styles['option-custom']} ${isCustomSelected ? styles['option-custom-selected'] : ''}`}
              onClick={() => {
        ...
        {/* Custom duration inputs — only visible when "Custom" is selected */}
        {isCustomSelected && (
        ...
        <div style={{ borderTop: '1px solid #e9ebed', paddingTop: '12px' }}>
          <FormField label="Timezone">
            <Select
              selectedOption={pendingTimezone}
              options={timezoneOptions}
              onChange={({ detail }) => setPendingTimezone(detail.selectedOption)}
            />
          </FormField>
        </div>
      </SpaceBetween>
  ...
  return (
    <DateRangePicker
      ...
      onChange={({ detail }) => {
        ...
        setSelectedTimezone(pendingTimezone);
      }}
      relativeOptions={[]}
...
export default function ThemedComponentsPage() {
  ...
  return (
    <div style={{ padding: 15 }}>
      <h1>Custom date range picker component</h1>
