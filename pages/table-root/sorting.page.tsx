...
// a priority number next to each caret). Only the primary sort column declares `aria-sort` (ARIA
// permits a single sorted column); secondary columns keep the visual caret + priority number only.
// Caret icons match classic Table: `caret-down` (sortable),
            ...
            <TableHead>
              <TableHeaderRow>
                {COLUMNS.map(({ key, label }) => {
                  ...
                  return (
                    <TableHeaderCell key={key} ariaSort={index === 0 && active ? active.direction : 'none'}>
                      <button
                        type="button"
