...
const suite: TestSuite = {
  ...
  tests: [
    ...
    ...[true, false].map<TestSuite>(renderWithPortal => ({
      ...
      tests: [
        {
          description: `inside table - renderWithPortal=${renderWithPortal}`,
        ...
        ...[
          ...
          description: `scenario - ${scenario} - renderWithPortal=${renderWithPortal}`,
        ...
        })),
        {
          ...
          setup: async ({ page, wrapper }) => {
            ...
            const popover = wrapper.find('#scenario-with-select').findPopover();
            await page.click(popover.findTrigger().toSelector());
            await page.click(popover.findContent().findSelect().findTrigger().toSelector());
          },
        },
        {
          description: `scenario - error (lazy-loading) - renderWithPortal=${renderWithPortal}`,
          path: 'popover/scenarios',
          screenshotType: 'viewport',
          setup: async ({ page }) => {
            if (renderWithPortal) {
        ...
        {
          description: `inline popover - closed - renderWithPortal=${renderWithPortal}`,
          path: 'popover/inline',
          screenshotType: 'screenshotArea',
          setup: async ({ page }) => {
            if (renderWithPortal) {
        ...
        {
          description: `inline popover - open - renderWithPortal=${renderWithPortal}`,
          path: 'popover/inline',
          screenshotType: 'screenshotArea',
          setup: async ({ page, wrapper }) => {
            if (renderWithPortal) {
            ...
            const popover = wrapper.findPopover();
            await page.click(popover.findTrigger().toSelector());
          },
        },
        ...Object.entries({
          ...
          description: `positioning - ${testName} - renderWithPortal=${renderWithPortal}`,
        ...
        {
          description: `positioning - flips to the bottom variant of the perpendicular direction, no scroll - renderWithPortal=${renderWithPortal}`,
          path: 'popover/positioning',
          screenshotType: 'viewport',
          setup: async ({ page }) => {
            if (renderWithPortal) {
